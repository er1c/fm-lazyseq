...
object Implicits {
  ...
  implicit class LazySeqRichFile(val file: File) extends AnyVal {
    ...
    def recursiveFiles(
      ...
      ignoreDotFiles: Boolean = true): LazySeq[File] = {
      recursiveFilesImpl(
        maxDepth,
        followSymLinks,
        includeFiles = true,
        includeDirs = false,
        depthFirstDirs = false,
     ...
     */
    def recursiveFilesAndDirs(
      maxDepth: Int = Int.MaxValue,
      followSymLinks: Boolean = false,
      ignoreDotFiles: Boolean = true,
      depthFirstDirs: Boolean = false): LazySeq[File] = {
      recursiveFilesImpl(
        maxDepth,
        followSymLinks,
        includeFiles = true,
    ...
    private def recursiveFilesImpl(
      maxDepth: Int,
      followSymLinks: Boolean,
      includeFiles: Boolean,
      includeDirs: Boolean,
      depthFirstDirs: Boolean,
      ignoreDotFiles: Boolean): LazySeq[File] =
      new LazySeq[File] {
        override def foreach[U](f: File => U): Unit = {
          ...
          val visitor = new SimpleFileVisitor[Path] {
            ...
            private def visitDir(dir: Path): Unit = {
              if (includeDirs) {
          ...
          val options: EnumSet[FileVisitOption] =
            if (followSymLinks) EnumSet.of(FileVisitOption.FOLLOW_LINKS) else EnumSet.noneOf(classOf[FileVisitOption])
