 ...
 */
final private class MergeCorrespondingLazySeq[L, R, K](
  leftReader: LazySeq[L],
  rightReader: LazySeq[R],
  toLeftKey: L => K,
  toRightKey: R => K)(implicit ord: Ordering[K])
  extends LazySeq[LazySeq.EitherOrBoth[L, R]] {
  private[this] val asserts: Boolean = true

  import LazySeq.{Both, EitherOrBoth, Left, Right}

  // This uses a thread for the rightReader to create the iterator
  override def foreach[U](f: EitherOrBoth[L, R] => U) =
    Resource(rightReader.toIterator()).use { rightIt =>
      ...
      leftReader.foreach { left: L =>
        if (asserts) {
          ...
          if (prevLeftKeyDefined)
            assert(
              ord.lt(prevLeftKey, leftKey),
              "Incorrect usage of MergeCorrespondingLazySeq.  Inputs are not sorted/unique!")
        ...
        if (!rightIt.hasNext) f(Left(left))
          ...
          while (rightIt.hasNext && ord.lt(toRightKey(rightIt.head), leftKey)) {
            ...
            if (asserts) {
              ...
              if (prevRightKeyDefined)
                assert(
                  ord.lt(prevRightKey, rightKey),
                  "Incorrect usage of MergeCorrespondingLazySeq.  Inputs are not sorted/unique!")
