...
object LazySeq extends LazySeqObjBase[LazySeq] {
  override def newBuilder[A]: Builder[A, LazySeq[A]] = new LazySeqBuilder[A]
  ...
  def wrap[T, U](foreach: (T => U) => Unit): LazySeq[T] =
    new ForeachLazySeq[T](foreach.asInstanceOf[(T => Any) => Unit])
  ...
  def build[T](queueSize: Int)(f: Growable[T] => Unit): LazySeq[T] =
    new LazySeqBuilder[T](queueSize).withProducerThread(f).lazySeq

  /**
   * Asynchronously build a LazySeq by spinning up a Producer thread and running the given function to produce elements
   */
  def build[T](f: Growable[T] => Unit): LazySeq[T] = new LazySeqBuilder[T]().withProducerThread(f).lazySeq

  /**
   * Combine multiple LazySeqs into a single one by sequentially reading each reader
   */
  def apply[T](readers: TraversableOnce[LazySeq[T]]): LazySeq[T] = new MultiLazySeq[T](readers.toSeq: _*)
   ...
   */
  def parCombine[T](
    readers: TraversableOnce[LazySeq[T]],
    threads: Int = LazySeq.defaultThreadCount,
    queueSize: Int = LazySeq.defaultThreadCount): LazySeq[T] = {
    new ParallelMultiLazySeq[T](threads = threads, queueSize = queueSize, readers.toSeq: _*)
  }
   ...
   */
  implicit def canBuildFrom[A, B]: CanBuildFrom[LazySeq[A], B, LazySeq[B]] =
    null.asInstanceOf[CanBuildFrom[LazySeq[A], B, LazySeq[B]]]

  /**
   * An empty resource reader
   */
  override def empty[T]: LazySeq[T] = EmptyLazySeq
...
trait LazySeq[+A] extends LazySeqBase[A] {
  // extends IterableOnce[A] with IterableOps[A, LazySeq, LazySeq[A]] {
  import LazySeq.breaks._
  //override def companion: GenericCompanion[LazySeq] = LazySeq
  ...
  def iterator: LazySeqIterator[A] = new BatchedLazySeqIterator(this)
   ...
   * A Parallel foreach
   */
  final def parForeach[U](threads: Int = LazySeq.defaultThreadCount, inputBuffer: Int = LazySeq.defaultThreadCount)(
    f: A => U): Unit = {
  ...
  final override def flatten[B](implicit asTraversable: A => GenTraversableOnce[B]): LazySeq[B] =
    flatMap { a => asTraversable(a) }
  ...
  final def grouped[B >: A](size: Int, additionalIncrement: B => Int): LazySeq[IndexedSeq[B]] =
    new GroupedLazySeq(this, size, additionalIncrement)

  final override def slice(from: Int, until: Int): LazySeq[A] = new SlicedLazySeq(this, from, until)
  final override def take(n: Int): LazySeq[A] = if (n === 0) LazySeq.empty else slice(0, n)
  final override def drop(n: Int): LazySeq[A] = if (n === 0) this else slice(n, Int.MaxValue)

  final override def takeWhile(p: A => Boolean): LazySeq[A] = new TakeWhileLazySeq(this, p)
  final override def dropWhile(p: A => Boolean): LazySeq[A] = new DropWhileLazySeq(this, p)

  final def sortBy[B >: A, K](serializer: Serializer[B])(f: B => K)(implicit ord: Ordering[K]): LazySeq[B] =
    new SortedLazySeq[B, K](this, f)(serializer, ord)
  final def sortBy[B >: A, K](f: B => K)(implicit serializer: Serializer[B], ord: Ordering[K]): LazySeq[B] =
    new SortedLazySeq[B, K](this, f)

  final def sortBy[B >: A, K](serializer: Serializer[B], bufferSizeLimitMB: Int, bufferRecordLimit: Int)(f: B => K)(
    implicit ord: Ordering[K]): LazySeq[B] =
    new SortedLazySeq[B, K](this, f, bufferSizeLimitMB = bufferSizeLimitMB, bufferRecordLimit = bufferRecordLimit)(
      serializer,
      ord)
  final def sortBy[B >: A, K](bufferSizeLimitMB: Int, bufferRecordLimit: Int)(
    f: B => K)(implicit serializer: Serializer[B], ord: Ordering[K]): LazySeq[B] =
    new SortedLazySeq[B, K](this, f, bufferSizeLimitMB = bufferSizeLimitMB, bufferRecordLimit = bufferRecordLimit)

  final def sorted[B >: A](implicit serializer: Serializer[B], ord: Ordering[B]): LazySeq[B] =
    new SortedLazySeq[B, B](this, (k: B) => k)
  final def sorted[B >: A](bufferSizeLimitMB: Int, bufferRecordLimit: Int)(implicit
    serializer: Serializer[B],
    ord: Ordering[B]): LazySeq[B] =
    new SortedLazySeq[B, B](
      this,
      (k: B) => k,
  ...
  final def shuffle[B >: A](random: Random)(implicit serializer: Serializer[B]): LazySeq[B] =
    new ShuffledLazySeq[B](this, random)(serializer)
  final def shuffle[B >: A](seed: Long)(implicit serializer: Serializer[B]): LazySeq[B] =
    new ShuffledLazySeq[B](this, new Random(seed))(serializer)
  ...
  final def assertSorted[B >: A](implicit ord: Ordering[B]): LazySeq[B] =
    new AssertSortedReader[B, B](this, unique = false, v => v)(ord)
  ...
  final def assertSortedAndUnique[B >: A](implicit ord: Ordering[B]): LazySeq[B] =
    new AssertSortedReader[B, B](this, unique = true, v => v)(ord)

  final def assertSortedBy[K](key: A => K)(implicit ord: Ordering[K]): LazySeq[A] =
    new AssertSortedReader[A, K](this, unique = false, key)(ord)
  final def assertSortedAndUniqueBy[K](key: A => K)(implicit ord: Ordering[K]): LazySeq[A] =
    new AssertSortedReader[A, K](this, unique = true, key)(ord)
   ...
   */
  final def sortAndCollapseBy[B >: A, K](serializer: Serializer[B])(key: B => K)(op: (B, B) => B)(implicit
    ord: Ordering[K]): LazySeq[B] = {
    new CollapsingLazySeq(sortBy(serializer)(key), key, op)
  }

  final def sortAndCollapseBy[B >: A, K](key: B => K)(
    op: (B, B) => B)(implicit serializer: Serializer[B], ord: Ordering[K]): LazySeq[B] = {
    new CollapsingLazySeq(sortBy(key), key, op)
  }

  final def sortAndCollapseBy[B >: A, K](serializer: Serializer[B], bufferSizeLimitMB: Int, bufferRecordLimit: Int)(
    key: B => K)(op: (B, B) => B)(implicit ord: Ordering[K]): LazySeq[B] = {
    new CollapsingLazySeq(
      sortBy(serializer, bufferSizeLimitMB = bufferSizeLimitMB, bufferRecordLimit = bufferRecordLimit)(key),
      key,
      op)
  }

  final def sortAndCollapseBy[B >: A, K](bufferSizeLimitMB: Int, bufferRecordLimit: Int)(key: B => K)(
    op: (B, B) => B)(implicit serializer: Serializer[B], ord: Ordering[K]): LazySeq[B] = {
    new CollapsingLazySeq(
      sortBy(serializer, bufferSizeLimitMB = bufferSizeLimitMB, bufferRecordLimit = bufferRecordLimit)(key),
      key,
  ...
  final def collapseBy[B >: A, K](key: B => K)(op: (B, B) => B): LazySeq[B] =
    new CollapsingLazySeq(this, key, op)
  ...
  final def parMap[B](
    threads: Int = LazySeq.defaultThreadCount,
    inputBuffer: Int = LazySeq.defaultThreadCount,
    resultBuffer: Int = LazySeq.defaultThreadCount * 4)(f: A => B): LazySeq[B] = {
    if (threads === 1) map(f)
   ...
   * Performs a parallel flat map maintaining ordered output
   */
  final def parFlatMap[B](
    threads: Int = LazySeq.defaultThreadCount,
    inputBuffer: Int = LazySeq.defaultThreadCount,
    resultBuffer: Int = LazySeq.defaultThreadCount * 4)(f: A => GenTraversableOnce[B]): LazySeq[B] = {
    if (threads === 1) flatMap(f)
    else parMap(threads, inputBuffer, resultBuffer)(f).flatten
  }

  final def mergeCorresponding[B >: A](that: LazySeq[B])(implicit
    ord: Ordering[B]): LazySeq[LazySeq.EitherOrBoth[B, B]] =
    new MergeCorrespondingLazySeq(this, that, (v: B) => v, (v: B) => v)(ord)
   ...
   */
  final def mergeCorrespondingByKey[R, K](that: LazySeq[R], thisKey: A => K, thatKey: R => K)(implicit
    ord: Ordering[K]): LazySeq[LazySeq.EitherOrBoth[A, R]] =
    new MergeCorrespondingLazySeq(this, that, thisKey, thatKey)(ord)
  ...
  final def beforeWithResource[R, U](resource: Resource[R])(f: (A, R) => U): LazySeq[A] =
    new BeforeAfterWithResourceLazySeq[A, R, U, Unit](this, resource, before = f)
  ...
  final def afterWithResource[R, U](resource: Resource[R])(f: (A, R) => U): LazySeq[A] =
    new BeforeAfterWithResourceLazySeq[A, R, Unit, U](this, resource, after = f)
  ...
  def toIterator(batchSize: Int = 32, bufferSize: Int = 0): LazySeqIterator[A] =
    new BatchedLazySeqIterator(this, batchSize = batchSize, bufferSize = bufferSize)
}
