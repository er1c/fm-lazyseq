...
final class SortedLazySeqBuilder[V, K](
...
)(implicit serializer: Serializer[V], ord: Ordering[K])
  extends BuilderBase[V] {
  import serializer._
  ...
  private[this] val sortAndSaveTaskRunner: TaskRunner =
    TaskRunner(name = "SortedLazySeq-SortAndSave", threads = sortAndSaveThreads, queueSize = sortAndSaveQueueSize)
  ...
  private def sortAndSave(buffer: Array[KeyBytesPair]): Vector[MappedByteBuffer] = {
    ...
    val os: DataOutputStream = new DataOutputStream(
  ...
  final class ReadSortedRecords(files: Vector[Vector[MappedByteBuffer]]) extends LazySeq[V] with Closeable {
    ...
    override def foreach[U](f: V => U): Unit =
      Resource.using(makeBufferedRecordReaders()) { readers =>
    ...
    private def makePriorityQueue() =
      new PriorityQueue[BufferedRecordReader](
        files.length,
  ...
  private final class BufferedRecordReader(bufs: Vector[MappedByteBuffer]) extends Closeable {
    private[this] val is: DataInputStream = new DataInputStream(
      Snappy.newSnappyOrGzipInputStream(ByteBufferInputStream(bufs)))
    ...
    private def readBytesFromInputStream(size: Int): Array[Byte] = {
      ...
      while (totalBytesRead < size) {
        ...
        if (-1 === n) {
          if (totalBytesRead > 0)
            logger.error(
              "Unexpected EOFException.  Expected to read " + size + " bytes but only got " + totalBytesRead + " bytes before EOF")
