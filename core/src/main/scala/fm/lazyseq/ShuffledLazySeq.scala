 ...
 */
final private class ShuffledLazySeq[V](reader: LazySeq[V], random: Random = new Random())(implicit
  ...
  private object KeyValueSerializer extends Serializer[KeyValue] {
    def serialize(value: KeyValue): Array[Byte] =
      ByteBuffer.allocate(8).putLong(value.key).array() ++ serializer.serialize(value.value)
  ...
  private def shuffledReader: LazySeq[V] =
    reader.map { v => KeyValue(random.nextLong(), v) }.sortBy(KeyValueSerializer) { _.key }.map { _.value }

  override def foreach[U](f: V => U): Unit = shuffledReader.foreach(f)
}
